                     ...
                     <tbody>
								<c:forEach items="${list}" var="recoreds" varStatus="s">
									<tr >
										...
										<td><c:out value="${recoreds.opponentName}" /></td>
										<td><c:out value="${recoreds.productName}" /></td>
										<td><c:out value="${recoreds.leaveFacPrice}" /></td>
										<td><c:out value="${recoreds.distance}" /></td>
										
										<td><c:out value="${recoreds.freight}" /></td>
										<td><c:out value="${recoreds.truePrice}" /></td>
										<td><c:out value="${recoreds.sellPrice}" /></td>
		    ...
		    <div class="modal-dialog">
		        <div class="modal-content">
		            ...
		            <div class="modal-footer">
						  <button  onclick="javascript:parent.location.href='marketInfo.jsp'"
			...
			success:function(data2){
				var data;
	                ...
	                if(data.resultcode == "200"){
	                	txtHtml="<p class='text-info'>市场删除成功</p>";
	                }else{
	                	txtHtml="<p class='text-warning'>市场删除失败 </p>";
	                }
	                $("#result").html(txtHtml);
    				$("#myModal").modal("show");
...
num) {
    ...
    var url="showMarket.action?byPage=ture&currentPage=" + num + "";
